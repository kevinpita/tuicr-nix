...
readonly GITHUB_REPO="agavra/tuicr"
...
set_src_hash() {
    local hash="$1"
    HASH="$hash" perl -0pi -e 's|(src = fetchFromGitHub \{.*?hash = )[^;]+;|$1"$ENV{HASH}";|s' "$PACKAGE_FILE"
}
...
set_cargo_hash() {
    local hash="$1"
    HASH="$hash" perl -0pi -e 's|cargoHash = [^;]+;|cargoHash = "$ENV{HASH}";|' "$PACKAGE_FILE"
}
...
verify_update() {
    ...
    log_info "Building tuicr"
    nix build "$(flake_ref)" --print-build-logs

    log_info "Checking tuicr version"
    local actual_version
    actual_version=$(./result/bin/tuicr --version)
    if [ "$actual_version" != "tuicr $version" ]; then
        log_error "Expected 'tuicr $version', got '$actual_version'"
        exit 1
    fi
}
...
update_to_version() {
    ...
    log_info "Updating tuicr from $current_version to $new_version"
